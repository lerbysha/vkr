...
{
	if (!is_grabbed && change_pos && length2(speed) > UNIGINE_EPSILON)
	{
		Vec3 pos = node->getPosition();
		Vec3 clamped_pos = clamp(pos, Vec3(handle_min_pos), Vec3(handle_max_pos));
		if (length2(clamped_pos - pos) > UNIGINE_EPSILON)
		{
		...
		else
		{
			...
			speed = lerp(speed, vec3::ZERO, saturate(ACCELERATION_FACTOR * Game::getIFps()));
		}
		if (body)
...
void ObjHandle::holdIt(VRPlayer* player, int hand_num)
{
	...
	if (change_rot)
	{
		...
		Visualizer::renderVector(node->getWorldPosition(), node->getWorldPosition() + old_rotate_vec, vec4::RED);
		Visualizer::renderVector(node->getWorldPosition(), node->getWorldPosition() + new_rotate_vec, vec4::GREEN);
		Visualizer::renderVector(node->getWorldPosition(), node->getWorldPosition() + up, vec4::BLUE);
