...
public:
	...
	PROP_PARAM(File, enable_sound_file);
	PROP_PARAM(File, loop_sound_file);
	PROP_PARAM(File, disable_sound_file);

	// interact methods
	void useIt(VRPlayer* player, int hand_num) override;

	void setDefault(int condition);
...
private:
	void init_sound(const char *file_name, SoundSourcePtr &sound_source);
	...
	Unigine::SoundSourcePtr enable_sound;
	Unigine::SoundSourcePtr loop_sound;
	Unigine::SoundSourcePtr disable_sound;
};