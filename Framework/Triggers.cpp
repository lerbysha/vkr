...
void Triggers::init(const NodePtr &node, float new_extra_scale)
{
	root = node;
	if (!root)
		return;

	extra_scale = new_extra_scale;
	trigger.clear();
	init_children(root);
}
...
void Triggers::addTrigger(const NodePtr &node)
{
	...
	if (obj)
	{
		...
		if (obj_body)
		{
			count = obj_body->getNumShapes();
			for (int i = 0; i < count; i++)
				obj_body->getShape(i)->setEnabled(false);
		}

		node->setScale(node->getScale() + vec3::ONE * extra_scale);
		node->setEnabled(0);
...
void Triggers::get_depth_box(int i, const vec3 &p, const Vec3 &pw)
{
	...
	float dx = static_cast<float>(length(trigger[i].node->toWorld(vec3(x, p.y, p.z)) - pw));
	float y = Math::abs(-0.5f - p.y) < Math::abs(0.5f - p.y) ? -0.5f : 0.5f;
	float dy = static_cast<float>(length(trigger[i].node->toWorld(vec3(p.x, y, p.z)) - pw));
	float z = Math::abs(-0.5f - p.z) < Math::abs(0.5f - p.z) ? -0.5f : 0.5f;
	float dz = static_cast<float>(length(trigger[i].node->toWorld(vec3(p.x, p.y, z)) - pw));
...
{
	float d = clamp(dot(p, vec3::UP), -0.5f, 0.5f); // height
	vec3 line_point = vec3::UP * d;
